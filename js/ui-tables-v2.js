function renderTopPanels(){
  ...
  document.getElementById("topCounties").innerHTML = topSeverity.map(c => rankRow(c, c.currentSeverity, maxSeverity, `${safeNum(c.customersOut).toLocaleString()} out · Forecast ${safeNum(c.forecastStormRisk)}`)).join("");
  ...
  document.getElementById("topPredicted").innerHTML = topPredicted.map(c => rankRow(c, c.blendedPredictedRisk || c.predictedRisk, maxPredicted, `${c.mlRiskBand || c.predictedRiskBand} · ML ${safeNum(c.mlRiskScore)} · Forecast ${safeNum(c.forecastStormRisk)}`)).join("");
}
